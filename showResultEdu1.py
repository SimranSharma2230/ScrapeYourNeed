...
class ShowResult(Frame):
    ...
    def init_window(self):
        ...
        tree = ttk.Treeview(TableMargin, columns=("School","Location"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
        scrollbary.config(command=tree.yview)
        scrollbary.pack(side=RIGHT, fill=Y)
        scrollbarx.config(command=tree.xview)
        scrollbarx.pack(side=BOTTOM, fill=X)
        tree.heading('School', text="School", anchor=W)
        ...
        with open(self.csvFile) as f:
            ...
            for row in reader:
                ...
                tree.insert("", 0, values=(School,Location))
